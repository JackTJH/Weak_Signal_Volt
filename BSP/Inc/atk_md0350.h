...
// #define LCD_BASE        0x6C0007FE      // 使用A10地址线
// #define LCD             ((LCD_TypeDef *) LCD_BASE)
...
/* 常用画笔颜色 */
#define WHITE           0xFFFF-0xFFFF      /* 白色 */
#define BLACK           0xFFFF-0x0000      /* 黑色 */
#define RED             0xFFFF-0xF800      /* 红色 */
#define GREEN           0xFFFF-0x07E0      /* 绿色 */
#define BLUE            0xFFFF-0x001F      /* 蓝色 */
#define MAGENTA         0xFFFF-0xF81F      /* 品红色/紫红色 = BLUE + RED */
#define YELLOW          0xFFFF-0xFFE0      /* 黄色 = GREEN + RED */
#define CYAN            0xFFFF-0x07FF      /* 青色 = GREEN + BLUE */
...
/* 字体大小 */
#define Word_Size_12 12
#define Word_Size_16 16
#define Word_Size_24 24
#define Word_Size_32 32
...
void lcd_draw_point(uint16_t x, uint16_t y, uint16_t color);/* 画点(32位颜色,兼容LTDC) */
...
void lcd_fill(uint16_t sx, uint16_t sy, uint16_t ex, uint16_t ey, uint16_t color);          /* 纯色填充矩形(32位颜色,兼容LTDC) */
...
void lcd_show_char(uint16_t x, uint16_t y, char chr, uint8_t size, uint8_t mode, uint16_t color, uint16_t back_color);                       /* 显示一个字符 */
void lcd_show_num(uint16_t x, uint16_t y, uint32_t num, uint8_t len, uint8_t size, uint16_t color, uint16_t back_color);                     /* 显示数字 */
void lcd_show_xnum(uint16_t x, uint16_t y, uint32_t num, uint8_t len, uint8_t size, uint8_t mode, uint16_t color, uint16_t back_color);      /* 扩展显示数字 */
void lcd_show_string_rigon(uint16_t x, uint16_t y, uint16_t width, uint16_t height, uint8_t size, char *p, uint16_t color, uint16_t back_color);   /* 显示字符串 */
